...
const svgCal = (
	...
	for (let week = 0; week < daysByWeek.length; week++) {
		...
		for (let day = 0; day < days.length; day++) {
			...
			//day box
			var dayBox = document.createElementNS("http://www.w3.org/2000/svg", "rect");
			...
			// console.log('start');
			// 	Window.db.window.collection("user").doc(firebase.auth().currentUser.uid).set(
			// 		{date:dayt},{merge: true}
			// 		);
			// 		console.log('end');
...
//use it!
let month = 0;
var today = new Date(new Date().getFullYear(), new Date().getMonth() + month, new Date().getDate());
